...
# Use CLOCK_CYCLES to define whether the elapsed clock cycles should be printed
# or not
CLOCK_CYCLES = False
# Use RUNTIME to define whether the runtime should be measured or not
RUNTIME = False
# Use RUNTIME_AES to define whether the runtime of a single run of AES should
# be measured or not
RUNTIME_AES = False
# Use RESULT to define whether the results should be printed or not
RESULT = True
# Use MESSAGE_LENGTH to define whether the message length should be 64 bytes
# (512 bits) or 128 bytes (1024 bits)
MESSAGE_LENGTH = 128
...
def logical_xor(str1, str2):
    ...
    # Convert the integer result to binarys
...
def mdc2(message, iv_A, iv_B, blocksize, use_aesni):
    ...
    for block in array:
        ...
        # Logical xor from the actual message block as bytes and the
        # encryption as bytes
        ...
        a = vL + wR
        b = wL + vR
        # Convert the binarys 'a' and 'b' to the byte-arry 'iv_A' and 'iv_B'
        # and pad it on the left with zeros until the length is blocksize * 2
...
iv_A = b'\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52'\
        b'\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52\x52'
# Initialization vector B: \x25 hex is a '%' as string
iv_B = b'\x25\x25\x25\x25\x25\x25\x25\x25\x25\x25\x25\x25\x25\x25\x25\x25'\
...
if CLOCK_CYCLES is True or RUNTIME is True or RESULT is True:
